...
apk update
apk add bird iptables nftables iproute2-ss openssh-server openssh-client nmap
rc-update add bird
rc-update add sshd
...
echo "You can access the vault machine at 100.80.0.4
It stores the confidential copy of the current novel
Access the machine with the following credentials :
debian:vault!

Once on the machine, you can read the novel by running the binary get_novel" > /root/README
