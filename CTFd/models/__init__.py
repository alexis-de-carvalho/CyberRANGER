...
class AccessTokens(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    value = db.Column(db.String(80), default="S3cr3tT0k3n")
    user_id = db.Column(db.Integer, db.ForeignKey("users.id", ondelete="CASCADE"))
    team_id = db.Column(db.Integer, db.ForeignKey("teams.id", ondelete="CASCADE"))
    instance_id = db.Column(db.Integer, db.ForeignKey("instances.id", ondelete="CASCADE"))
    machine = db.Column(db.String(80))
    network = db.Column(db.String(80))
    user = db.relationship("Users", foreign_keys="AccessTokens.user_id", lazy="select")
    team = db.relationship("Teams", foreign_keys="AccessTokens.team_id", lazy="select")
    instance = db.relationship("Instances", foreign_keys="AccessTokens.instance_id", lazy="select")

    def __init__(self, *args, **kwargs):
        super(AcessTokens, self).__init__(**kwargs)
...
class Instances(db.Model):
    ...
    challenge = db.relationship("Challenges", foreign_keys="Instances.challenge_id", lazy="select")
    
    def __init__(self, *args, **kwargs):
        super(Instances, self).__init__(**kwargs)
